 ...
 * 构造data结构及数据
 */
const structureData = (data: number) => {
  const item = [count.value, parseInt((data/5).toFixed(1)), data];
  echartsData.push(item);
  echartsData = echartsData.map(function (val) {
...
const initTemperatureEcharts = () => {
  let myChart = echarts.getInstanceByDom(document.getElementById("my-chart"));
  if (myChart == null) {
    myChart = echarts.init(echartsTemperatureRef.value);
  }
  const option = {
    title: {
      show: true,
...
const onMessage = (event: any) => {
  ...
  structureData(parseInt(event.data));
}
  ...
  <section class="temperature sections">
    ...
    <div class="body">
      <div class="echarts-wrap echarts-temperature">
        <div class="canvas-wrap" ref="echartsTemperatureRef" id="my-chart" style="width: 560px; height: 373px;" />
      </div>
    </div>
  </section>
